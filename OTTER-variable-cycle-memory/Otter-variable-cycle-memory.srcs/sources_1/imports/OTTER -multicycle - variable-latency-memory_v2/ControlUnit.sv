...
module OTTER_CU_FSM(
    ...
    input CU_prevINT,
    input CU_RESET,
    input CU_MEM_READY,  // ADDED FOR Variable Cycle Memory
    input CU_MEM_VALID,
    input [6:0] CU_OPCODE,
    input [2:0] CU_FUNC3,
    ...
    output logic CU_intCLR
       ...
       //DECODING (depends on state) ////////////////////////////////////////////////////
        ...
        wire stall = (state == EXECUTE && (CU_OPCODE == LOAD || CU_OPCODE ==STORE) && !CU_MEM_READY) || (state == WB && !CU_MEM_VALID);
        assign CU_PCWRITE = !stall && ((state==1 && CU_OPCODE!=LOAD)|| (state==2 && CU_OPCODE==LOAD)||(state==INTER)); //(state == 0) || (state==1 && (CU_OPCODE ==JAL || CU_OPCODE==JALR || (CU_OPCODE==BRANCH && brn_cond)));
    ...
    begin // all state updates on a positive clock edge
            ...
            else if (!stall) begin         // ADDED "if (!CU_STALL)" FOR VLM SYSTEM
                case (state)       
